...
def index(request):
    # JST 22:00-6:00(UTC 13:00-21:00)は稼働停止時間とする。
...
#    dt_st2 = datetime.time(6, 35, 0)  # JST
    ...
    return render(request, 'BesideMon202211/index.html', params)
    ...
    if request.method == 'POST':  # POST
        ...
        if user:  # ユーザー認証
            ...
            if user.is_active:  # ログイン
                login(request, user)
                # ホームページ遷移
                return HttpResponseRedirect(reverse('BesideMon202211:index1'))  # reverse関数(django.urls.reverse) : 関数名からURLを逆引き
            else:  # アカウント利用不可
...
def viewslogout(request):
    logout(request)
    return HttpResponseRedirect(reverse('BesideMon202211:viewslogin'))  # ログイン画面遷移